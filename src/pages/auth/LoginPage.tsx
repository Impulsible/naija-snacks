...
export const LoginPage: React.FC = () => {
  ...
  const location = useLocation(); // <--- Added this to capture protected route origin
  const { login } = useAuth();
  ...
  const onSubmit = async (data: LoginSchema) => {
    ...
    try {
      ...
      // Look up saved user safely to determine routing
      const savedUser = JSON.parse(localStorage.getItem('user') || '{}');
      const role = (savedUser?.role || '').toString().toLowerCase();
