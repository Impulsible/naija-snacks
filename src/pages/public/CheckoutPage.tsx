...
const CheckoutPage = () => {
  ...
  const onSubmit = async (data: CheckoutFormData) => {
    ...
    try {
      ...
      // Support both flat and wrapped order response structures safely
      const orderId = (result as any).id || (result as any)._id || (result as any).order?.id || (result as any).order?._id;

      if (!orderId) {
        throw new Error('Could not retrieve order reference from backend.');
      }

      clearCart();

      if (data.paymentMethod === 'paystack') {
        const payment = await orderService.initializePayment(orderId);
    ...
    <div className="min-h-screen bg-[#FFFDF9] py-6 sm:py-10 lg:py-14 text-stone-900">
      <Container>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="relative">
                ...
                <div className="space-y-3">
                  ...
                  <label className={`relative flex items-start gap-4 p-4 sm:p-5 rounded-2xl border-2 cursor-pointer transition-all ${
                    ...
                    <div className={`mt-0.5 shrink-0 w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${
                      ...
                      {paymentMethod === 'cash_on_delivery' && <div className="w-2.5 h-2.5 bg-primary rounded-full" />}
                    </div>
                    <div className="flex-1 min-w-0">
              ...
              <div className="bg-white rounded-3xl shadow-sm border border-amber-950/10 p-5 sm:p-6 lg:sticky lg:top-24">
                    ...
                    <div key={item.product.id} className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[10px] text-stone-500 mt-0.5 truncate">
                          {typeof item.product.category === 'string' ? item.product.category.replace(/-/g, ' ') : ''}
                        </p>
                      </div>
                ...
                <div className="space-y-3 text-xs font-semibold text-stone-600 pt-4 border-t border-amber-950/5">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    {isFreeDelivery ? (
                      <span className="text-emerald-700 font-black bg-emerald-50 px-2 py-0.5 rounded-md">FREE</span>
                    ) : (
                  ...
                  {!isFreeDelivery && (
                    <div className="text-[10px] font-bold text-amber-700 bg-amber-50 p-2.5 rounded-xl border border-amber-200/50 flex items-center gap-1.5">
                      ...
                      <span>Add {formatCurrency(FREE_DELIVERY_THRESHOLD - rawSubtotal)} more for free delivery!</span>
                    </div>
                  )}
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="hidden lg:flex items-center justify-center gap-2 w-full mt-6 py-4 rounded-2xl bg-gradient-to-r from-stone-900 to-stone-800 hover:from-black hover:to-stone-900 text-white text-sm font-black shadow-xl transition-all disabled:opacity-70"
                >
                  {isLoading ? (
          ...
          <div className="lg:hidden fixed bottom-0 left-0 right-0 p-4 bg-[#FFFDF9]/95 backdrop-blur-xl border-t border-amber-950/10 z-40">
            <button
              type="submit"
              disabled={isLoading}
              className="flex items-center justify-between w-full px-5 py-4 rounded-2xl bg-stone-900 text-white shadow-lg disabled:opacity-70"
            >
              ...
              <span className="font-heading font-black text-lg">{formatCurrency(total)}</span>
            </button>
          </div>
        </form>
      </Container>
